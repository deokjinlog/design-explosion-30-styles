...
① **스타일 금지 검사** — `python3 "${CLAUDE_PLUGIN_ROOT}/skills/design-style-explorer/scripts/style-lint.py" <파일 또는 배치폴더>`
그 스타일의 금지(라운드·그림자·그라디언트…) 위반을 검사한다. **FAIL 이면 해당 스타일 파일을 다시 읽고 수정 → 재린트** (생성→린트→수정→재린트 루프). `[MANUAL]` 항목은 크리틱 루프에서 직접 확인한다.

② **원형 적합성 검사** — `python3 "${CLAUDE_PLUGIN_ROOT}/skills/design-style-explorer/scripts/archetype-lint.py" <배치폴더>`
