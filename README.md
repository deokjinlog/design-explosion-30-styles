...
>
> 도메인 하나만 던지면 **같은 화면을 30가지 디자인 스타일로 폭발**시켜요.
...
이 도구는 그걸 **30갈래로 강제로 벌려놓고 눈으로 비교**하게 만들어요.

<br/>

## ⚡ 뭐가 다른가요?

|  | 그냥 AI한테 "디자인 해줘" | 🎨 **design-explosion** |
|---|---|---|
...
1. **콘텐츠는 고정, 시각 언어만 바꾼다** — 순수하게 디자인만 비교되게.
2. **스타일마다 각본이 있다** — 무작위 생성이 아니라 전용 프롬프트로 방향을 강제.
3. **금지를 린트로 강제한다** — "이 스타일에선 이건 반칙"을 코드가 잡는다.
4. **최악 콘텐츠로 테스트한다** — 예쁜 더미가 아니라 실제로 무너지는 조건으로.
5. **정해지면 시스템으로 굳힌다** — 한 번 고른 방향은 토큰·규칙으로 재사용.
...
이 프로젝트는 [**dj-superkit**](https://github.com/deokjinlog/dj-superkit)(*AI는 빠르게, 코드는 안전하게* — 프로덕션 코드 워크플로)에서 갈라져 나온 독립 디자인 도구예요. 코드를 안전하게 만드는 일과 디자인 방향을 탐색하는 일은 다른 작업이라 각자의 저장소로 나눴어요. 서로 의존하지 않으니 **필요한 것만** 설치해 쓰면 됩니다.
...
<p align="center">
  <sub>디자인은 <b>"하나 뽑아 결정"</b>이 아니라 <b>"여럿 벌려놓고 고르기"</b>.</sub>
</p>

<p align="center">
  <sub>MIT · 버그 / 제안: <a href="https://github.com/deokjinlog/design-explosion-30-styles/issues">GitHub Issues</a></sub>
</p>
